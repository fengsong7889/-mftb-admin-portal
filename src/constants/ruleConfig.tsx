...
export const DEFAULT_RULE_GROUPS: RuleGroup[] = [
  ...
  },
  {
    ...
    rules: [
      ...
      { key: 'eam_consumable_item', label: '耗材編碼', type: 'table', value: 'HC', defaultValue: 'HC', dateFormat: '', min: 6, max: 6, unit: 'HC000001', remark: '{prefix} + {n}位數字自增（全局自增）', menu: '物資管理(EAM)-耗材檔案' },
      { key: 'eam_consumable_claim', label: '耗材領用單號', type: 'table', value: 'HCLY', defaultValue: 'HCLY', dateFormat: 'YYYYMMDD', min: 4, max: 4, unit: 'HCLY202609180000', remark: '{prefix} + YYYYMMDD + {n}位自增序號', menu: '物資管理(EAM)-耗材領用' },
      ...
      /* ── 物資管理（EAM）── 基礎配置 / 供應商管理 ── */
      { key: 'eam_category_l1', label: '資產分類編碼-一級', type: 'table', value: '-', defaultValue: '-', dateFormat: '', min: 2, max: 2, unit: '01', remark: '2位數字自增，同級最大序號+1（如 01、02、03）', menu: '物資管理(EAM)-基礎配置' },
      { key: 'eam_category_l2', label: '資產分類編碼-二級', type: 'table', value: '-', defaultValue: '-', dateFormat: '', min: 2, max: 2, unit: '01-01', remark: '父編碼 +「-」+ 2位數字自增（如 01-01、01-02）', menu: '物資管理(EAM)-基礎配置' },
      { key: 'eam_category_l3', label: '資產分類編碼-三級', type: 'table', value: '-', defaultValue: '-', dateFormat: '', min: 2, max: 2, unit: '01-01-01', remark: '父編碼 +「-」+ 2位數字自增（如 01-01-01、01-01-02）', menu: '物資管理(EAM)-基礎配置' },
      { key: 'eam_location_warehouse', label: '倉庫編碼', type: 'table', value: '', defaultValue: '', dateFormat: '', min: 1, max: 10, unit: 'M', remark: '人工手動輸入編碼（如 M、T、Z），無固定格式', menu: '物資管理(EAM)-基礎配置' },
      { key: 'eam_supplier_code', label: '供應商編碼', type: 'table', value: 'CGSJ', defaultValue: 'CGSJ', dateFormat: '', min: 6, max: 6, unit: 'CGSJ000001', remark: '{prefix} + 6位數字自增（全局自增，如 CGSJ000001、CGSJ000002）', menu: '物資管理(EAM)-供應商管理' },
      { key: 'eam_claim', label: '領用編號', type: 'table', value: 'LY', defaultValue: 'LY', dateFormat: 'YYYYMMDD', min: 4, max: 4, unit: 'LY202609170001', remark: '{prefix} + YYYYMMDD + {n}位自增序號', menu: '物資管理(EAM)-領用資產' },
      { key: 'eam_return', label: '歸還編號', type: 'table', value: 'GH', defaultValue: 'GH', dateFormat: 'YYYYMMDD', min: 4, max: 4, unit: 'GH202609170001', remark: '{prefix} + YYYYMMDD + {n}位自增序號', menu: '物資管理(EAM)-資產歸還' },
      { key: 'eam_borrow', label: '借用編號', type: 'table', value: 'JY', defaultValue: 'JY', dateFormat: 'YYYYMMDD', min: 4, max: 4, unit: 'JY202609170001', remark: '{prefix} + YYYYMMDD + {n}位自增序號', menu: '物資管理(EAM)-借用資產' },
