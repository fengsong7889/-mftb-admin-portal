...
/** 構建時間戳：versionFilePlugin 生成檔案時與 define 共用同一值，確保前端對比一致 */
const BUILD_TIME = new Date().toISOString()

/**
 * 版本檢測插件：
 * 1. 生成 public/version.json — 前端定期拉取與內嵌時間戳對比
 * 2. 生成 src/build-meta.ts — 前端直接 import 取得構建時的 buildTime
 */
function versionFilePlugin(): Plugin {
  return {
    ...
    buildStart() {
      ...
      // public/version.json：供前端 fetch 對比
      const publicDir = path.resolve(__dirname, 'public')
      fs.mkdirSync(publicDir, { recursive: true })
      fs.writeFileSync(
        path.resolve(publicDir, 'version.json'),
        JSON.stringify({ buildTime: BUILD_TIME, commitHash }, null, 2),
      )

      // src/build-meta.ts：供前端直接 import
      fs.writeFileSync(
        path.resolve(__dirname, 'src/build-meta.ts'),
        `// Auto-generated by versionFilePlugin — DO NOT EDIT\nexport const BUILD_TIME = '${BUILD_TIME}'\nexport const COMMIT_HASH = '${commitHash}'\n`,
      )
    },
  }
}
...
export default defineConfig({
  plugins: [react(), llmProxyPlugin(__dirname), versionFilePlugin()],
