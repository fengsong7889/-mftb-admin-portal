...
public class FinAccountController {
    ...
    public Result<Void> freeze(@PathVariable String groupId) {
        finAccountService.freeze(groupId);
        return Result.success("账户已冻结", null);
    }

    /** 解冻账户 */
    @PutMapping("/{groupId}/unfreeze")
    public Result<Void> unfreeze(@PathVariable String groupId) {
        finAccountService.unfreeze(groupId);
