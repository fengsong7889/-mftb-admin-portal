...
public class AiQuotaOverrideDataInitializer implements CommandLineRunner {

    private static final String INIT_SCRIPT = "106_ai_quota_override.sql";
    ...
    public void run(String... args) {
        try {
            versionTracker.applyOnce("ai_quota_override:" + INIT_SCRIPT + ":v1", () -> {
                try {
            ...
            log.error("AI獨立額度表遷移失敗: {}", e.getMessage(), e);
        }
    }
            ...
            for (String stmt : noComment.split(";")) {
                ...
                if (!trimmed.isEmpty()) {
                    try {
                        ...
                        // 幂等处理：列/表已存在时忽略错误
                        log.debug("SQL 語句執行異常（可忽略）: {} — 原因: {}",
                                trimmed.substring(0, Math.min(trimmed.length(), 80)), e.getMessage());
                    }
                }
            }
            log.info("已執行 {} — 員工獨立額度表與申請表擴展字段遷移完成", scriptName);
        }
    }
}
